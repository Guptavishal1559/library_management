...
class Stock(models.Model):
    ...
    price = fields.Float('Price', related="book_id.book_price", group_operator='Avg')
    remaining_qty = fields.Integer('Remaining Quantity', compute="_cal_remaining_qty")
    total_qty = fields.Integer('Total Quantity', related="book_id.quantity")
    books_ids = fields.One2many('book.dept', 'stock_id', string='Add Book', ondelessste="cascade")

    # exercise2-Q1============================================================================
    language = fields.Many2one('book.language', 'Language', related="book_id.language_id", store=True)

    # exercise2-Q3============================================================================
    book_ids = fields.Many2many('book.type', string='Genre', related="book_id.genre")
    ...
    author_ids = fields.Many2many('author.details', string='Author Name', domain=[('name', '=', 'active_id')])

    parent_id = fields.Many2one('library.stock', 'Parent')
    ...
    @api.depends('remaining_qty', 'total_qty')
    def _cal_remaining_qty(self):
        """
        ...
        # print('Remaining Qty----------------------------->',self.remaining_qty)
        # print('Total Qty---------------------------------->',self.total_qty)
        for stock in self:
            stock.remaining_qty = stock.total_qty

        # active_records = self.filtered('book_id')
    ...
    def onchange_book_id(self):
        for rec in self:
            lines = []
        for line in self.book_id.author_ids:
            val = {
                'name': line.name,
